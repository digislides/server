...
@GenController(path: '/api/media/font')
class MediaFontRoutes extends Controller {
  /// Route to create a new program
  @PostJson()
  Future<MediaFont> create(Context ctx, Db db, ServerUser user) async {
    // TODO validate
    ...
    final accessor = MediaFontAccessor(db);
  ...
  Future<MediaFont> save(
    ...
    MediaFont info = await accessor.get(id);
    if (info == null) {
  ...
  */
  ...
  @GetJson()
  Future<List<MediaFont>> getAll(Context ctx, Db db, ServerUser user) async {
    final accessor = MediaFontAccessor(db);
